...
get_version() {
    ...
    if [[ -z "$VERSION" ]]; then
        ...
        if ! curl ${PROXY} -o "$TMP_FILE" 'https://api.github.com/repos/mayilab/v2ray-poseidon/releases/latest'; then
            rm "$TMP_FILE"
...
download_v2ray() {
    mkdir "$TMP_DIRECTORY"
    DOWNLOAD_LINK="https://github.com/mayilab/v2ray-poseidon/releases/download/$RELEASE_VERSION/v2ray-linux-$MACHINE.zip"
